...
def strassen(matrix1, matrix2):
    ...
    result = [C11[i] + C12[i] for i in range(len(C11))] + [C21[i] + C22[i] for i in range(len(C21))]
...
def benchmark(matrix_size, repetitions=5, warm_up_runs=3, seed=None):
    ...
    for _ in range(warm_up_runs):
        ...
        strassen(matrix1, matrix2)  # Warm-up to initialize the system
    ...
    average_runtimes = df.groupby('Matrix Size')['Runtime (seconds)'].mean().reset_index()
    print("\nAverage Runtimes:\n", average_runtimes)
